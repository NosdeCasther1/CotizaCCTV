...
class MySQLMCPServer {
            ...
            try {
                switch (name) {
                    ...
                    case "execute_read_query":
                        return await this.handleReadQuery(args);
                    default:
    ...
    async handleReadQuery(args) {
        const { query } = args;
        const normalizedQuery = query.trim().toUpperCase();
        // Strict Security Validation
        if (!normalizedQuery.startsWith("SELECT")) {
            throw new McpError(ErrorCode.InvalidParams, "Security Error: Only SELECT queries are allowed. Query must start with 'SELECT'.");
        }
        const forbiddenKeywords = [/INSERT/i, /UPDATE/i, /DELETE/i, /DROP/i, /ALTER/i, /TRUNCATE/i];
        for (const kw of forbiddenKeywords) {
            if (kw.test(query)) {
                throw new McpError(ErrorCode.InvalidParams, `Security Error: Query contains forbidden keyword '${kw.source}'.`);
            }
        }
        try {
