...
class GitMCPServer {
    server;
    SEMANTIC_REGEX = /^(feat|fix|docs|style|refactor|perf|test|build|ci|chore|revert)(\(.+\))?:\s.+/;
    constructor() {
     ...
     */
    async getGitClient(repoPath) {
        const resolvedPath = path.resolve(repoPath);
        if (!fs.existsSync(resolvedPath)) {
            throw new McpError(ErrorCode.InvalidParams, `Directory does not exist: ${resolvedPath}`);
        }
        const git = simpleGit(resolvedPath);
        const isRepo = await git.checkIsRepo();
        if (!isRepo) {
            throw new McpError(ErrorCode.InvalidParams, `Directory is not a valid Git repository: ${resolvedPath}`);
        }
        return git;
    }
    setupHandlers() {
            ...
            try {
                switch (name) {
                    ...
                    case "generate_semantic_commit":
                        return await this.handleSemanticCommit(args);
                    case "push_changes":
                        return await this.handlePushChanges(args);
                    case "discard_changes":
                        return await this.handleDiscardChanges(args);
                    default:
    ...
    async handleSemanticCommit(args) {
    ...
    async handleDiscardChanges(args) {
        const git = await this.getGitClient(args.repo_path);
