...
class BridgeMCPServer {
      ...
      try {
        switch (name) {
          case "test_api_endpoint":
            return await this.handleTestEndpoint(args as any);
          case "generate_ts_interfaces":
            return await this.handleGenerateInterface(args as any);
  ...
  private async handleTestEndpoint(args: { method: any; url: string; headers?: any; body?: any }) {
    try {
  ...
  private handleGenerateInterface(args: { json_payload: any; interface_name: string }) {
  ...
  private async handleWriteFile(args: { frontend_path: string; content: string }) {
