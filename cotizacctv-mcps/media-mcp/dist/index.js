...
class MediaMCPServer {
            ...
            try {
                switch (name) {
                    ...
                    case "organize_catalog_directory":
                        return await this.handleOrganizeDirectory(args);
                    default:
    ...
    async handleOptimizeImage(args) {
        ...
        if (!this.SUPPORTED_FORMATS.includes(ext)) {
            throw new McpError(ErrorCode.InvalidParams, `Unsupported image format: ${ext}. Supported: ${this.SUPPORTED_FORMATS.join(", ")}`);
        }
        try {
    ...
    async handleOrganizeDirectory(args) {
        const { base_directory, provider_name, category_name } = args;
        // Structure: public/storage/products/{provider}/{category}
        const targetPath = path.join(base_directory, "public", "storage", "products", provider_name.toLowerCase().replace(/\s+/g, "_"), category_name.toLowerCase().replace(/\s+/g, "_"));
        try {
